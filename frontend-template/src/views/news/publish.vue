        ...
        <el-table-column label="操作" width="200">
          <template #default="scope">
            <el-button-group>
              ...
              <el-button size="small" type="success" @click="handlePublish(scope.row.id)">发布</el-button>
            </el-button-group>
          </template>
        </el-table-column>
      </el-table>
...
import { getNewsListsUsingPost, publishNewsUsingPut } from '@/api/newsController'
...
const handlePublish = (id: string) => {
  ElMessageBox.confirm('确定要发布该新闻吗?', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    await publishNewsUsingPut({ id: id })
    fetchData()
    // TODO: 调用API通过审核
    ElMessage.success('发布通过')
